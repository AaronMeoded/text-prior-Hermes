...
class Hermes_UNet(nn.Module):
    def __init__(
        ...
        Args:
            ...
            scale: downsample scales per level
            kernel_size: conv kernel sizes per level
            block: block type name
            num_block: blocks per stage
            pool: use pooling vs strided conv
            norm: normalization type
            tn: number of task priors
            mn: number of modality priors
            embed_dim: dimension of text embeddings
            text_prior_num: number of text-prior tokens per fusion stage
        '''
        ...
        self.prior_init_fuse_2 = PriorInitFusionLayer(4*base_ch, 4*base_ch,
                                                     block_num=2, task_prior_num=tn, modality_prior_num=mn)
        ...
        self.prior_init_fuse_3 = PriorInitFusionLayer(8*base_ch, 8*base_ch,
                                                     block_num=2, task_prior_num=tn, modality_prior_num=mn)
        ...
        self.prior_init_fuse_4 = PriorInitFusionLayer(10*base_ch, 10*base_ch,
                                                     block_num=4, task_prior_num=tn, modality_prior_num=mn)
        ...
        self.prior_fuse_5 = PriorInitFusionLayer(8*base_ch, 8*base_ch,
                                                 block_num=2, task_prior_num=tn, modality_prior_num=mn)
        ...
        self.prior_fuse_6 = PriorInitFusionLayer(4*base_ch, 4*base_ch,
                                                 block_num=2, task_prior_num=tn, modality_prior_num=mn)
        ...
        # Simple linear heads: embed_dim -> text_prior_num * prior_dim per scale
    ...
    def forward(self, x, tgt_idx, mod_idx, raw_text_embeds):
        ...
        # Compute dynamic text-prior tokens for each fusion stage
        ...
        # Segmentation: task priors 0:tn
        task_list = [p[:, :tn, :] for p in [priors_2, priors_3, priors_4, priors_5, priors_6]]
        seg_out = self.out(out, task_list)

        # Modality: tokens tn:tn+mn
        mod_list = [p[:, tn:tn+mn, :] for p in [priors_2, priors_3, priors_4, priors_5, priors_6]]
